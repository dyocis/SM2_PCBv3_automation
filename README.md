...
> [!CAUTION]
> **`v0.1.0` release warning:** This release assumes the Peltier cooler, UV LEDs, and exhaust servo are installed. If you do not have these optional add-ons, do not install `v0.1.0`. The optional-hardware installer documented below is the next-release implementation on `main` and is planned for `v0.2.0`.
...
This installer registers a `stable` Git repository updater with Moonraker. After the first tagged release, updates appear in the Mainsail or Fluidd update manager. Shared configuration and dashboard files update from Git; `SM2_Local_Hardware.cfg`, its optional-hardware selections, and saved calibration state stay local.
